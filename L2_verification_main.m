...
[W, W0] = manufactured_solution_set_up(L, xs, ys, zs);

Uexact = prim_to_cons(gamma, W(1,:), W(2,:), W(5,:));
U = prim_to_cons(gamma, W0(1,:), W0(2,:), W0(5,:));
[ULBC, URBC] = manufactured_solution_BCs(U);
U = [ULBC, U, URBC];
Uexact_BCs = [ULBC, Uexact, URBC];
...
plot_vars(U,Uexact,xs);

function plot_vars(U,Uexact,xs,step)
gamma = 1.4;
[r, u, p, a, H, e, m, s] = cons_to_prim(U(:,4:end-3), gamma);
% [r, u, p, a, H, e, m, s] = cons_to_prim(U, gamma);
[re, ue, pe, ae, He, ee, me, se] = cons_to_prim(Uexact, gamma);
